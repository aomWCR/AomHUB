local Music = Instance.new("Sound",game:GetService("Players").LocalPlayer.PlayerGui)
Music.SoundId = "rbxassetid://4932456113"
Music.Playing = false
Music.Looped = true
Music.Volume = 1
Music.Pitch = 1
...
local library = {count = 0, queue = {}, callbacks = {}, rainbowtable = {} , toggled = true, binds = {},Music = Music,MusicSettings = {
	...
	ID = {
		...
		["กูมันแย่"] = 5342533283;
		["รู้ทั้งรู้"] = 2020538384;
		["แตกพ่องมึงอ่ะ"] = 1722145637;
	};
	Name = {
		"oki doki boomer";
		"Day n Nite";
...
function getWaitTime()
	return ToggleValue and math.clamp(library.Damage/5, 0, 2) or .2;
end

function getAbilityWaitTime()
	return getWaitTime() * 2 -- couple hits per ability use
end

function applyDebounce(mob)
	if (mob) then
		deb[mob] = true;
		wait(getWaitTime());
		deb[mob] = false;
...
function applySkillDebounce(mob, method)
	if (mob and method) then
		if skill_deb[mob] == nil then skill_deb[mob] = {} end;
		skill_deb[mob][method] = true;
		wait(getAbilityWaitTime());
		skill_deb[mob][method] = false;
	end
end

function massFire(args, aura)
	
	if (not args[3][3]) then
		return false
	end
	
	if (deb[args[3][3]]) then
		return false
	end;
	
	if (library.Damage > 1 or aura) then
		local lmb = args[3][2] == nil;
		local mult = ToggleValue and library.Damage or 1
		local hits = 0
		
		for i = 1, math.floor(mult) do
			...
			local function check()
				return mob.Entity.Health.Value <= mob.Entity.Health.MinValue;
			end
			
			local sbreak = check();
			if sbreak then break end;
			
			spawn(function()
				
				if ((library.SwordSkillsValue or not lmb)) then
			...
			spawn(function()
				if (lmb) then
					if check() then sbreak=true; return; end;
					if (hits % 10 == 0 and hits > 0 and mult > 1) then
						wait(1)
					end
					args[3][4] = genKey()
					args[3][2] = nil;
...
function hitAura()
	...
	local Target;
	local MobFolder = workspace:WaitForChild'Mobs':GetChildren();
	
	local Targets = {}
	
	if (library.AuraSet == 'Mobs') then
		Targets = MobFolder
	else
		for _,v in next, MobFolder do table.insert(Targets, v) end;
	end
	
	local Root = game.Players.LocalPlayer.Character:WaitForChild('HumanoidRootPart', 2) -- :C
	for _,Mob in next, Targets do
		local PrimaryPartCF = (Mob:IsA('Model') and Mob.PrimaryPart) and Mob:GetPrimaryPartCFrame()
		if (PrimaryPartCF) then
			local Distance = (Root.Position - PrimaryPartCF.p).Magnitude
			if (Distance <= 10 and Distance < Closest and not deb[Mob]) then
				Target = Mob;
				Closest = Distance;
			end
		end
	...
	if (Target and not deb[Target]) then
		spawn(function()
			...
			if(massFire(Args, true)) then
				coroutine.wrap(applyDebounce)(Target);
			end
...
local defaults; do
	...
	local types = {}; do
		...
		function types:Button2(name, callback)
			callback = callback or function() end;
